...
現在の配布ファイルには有料のコード署名証明書を使用していません。そのため、初回起動時にWindows SmartScreenから「WindowsによってPCが保護されました」または「認識されないアプリ」と表示される場合があります。この表示は、署名やダウンロード実績がまだないアプリに対する警告であり、それだけでウイルスが検出されたことを意味するものではありません。

必ずこのリポジトリの[公式Releasesページ](https://github.com/sorilin/stream-pitapon/releases)からダウンロードしたファイルであることを確認してから、次の操作を行ってください。

1. 警告画面の「詳細情報」をクリックします。
2. 発行元が「不明な発行元」と表示されていることを確認します。
3. ファイル名が `Stream-PitaPon-...exe` であることを確認します。
4. 問題がなければ「実行」をクリックします。

「詳細情報」や「実行」が表示されない場合は、無理にWindowsのセキュリティ機能全体を無効化せず、ファイルを削除して公式Releasesページから再度ダウンロードしてください。不安がある場合は実行しないでください。
